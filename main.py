...
#     print('loading model.....')
    if engine != 'deepcut':
        ...
        if mode == 'dg':
            ...
            model_load.load_weights(os.path.join(PATH,'model/lst20_rmsprop_LSTM_Attension_224_32_weight.h5'))
            # model_load.load_weights(os.path.join(PATH,f'model/{engine}_{opt_name}_{mode}_{lstm_node}_{attension_node}_weight.h5'))
            # return model_load
...
if __name__ == '__main__':
    text = 'การบ้านวันนี้มีอะไรบ้าง'
    load_model(engine='lst20')
    result = OSKut(text)
    print(result)